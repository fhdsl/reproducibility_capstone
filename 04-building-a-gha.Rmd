...
<input type="checkbox"> Locate the `ASSIGNMENT_2.yml` file within the `ASSIGNMENT_2` directory - this is your starting template <br/>
<input type="checkbox"> Review the existing template to understand what's already provided <br/>

::: {.notice}
**Important:** Do not change the filename from `ASSIGNMENT_2.yml` throughout the assignment. You will be moving the file's location, but do not change it's name.
:::
...
<input type="checkbox"> Move `ASSIGNMENT_2.yml` to `.github/workflows/ASSIGNMENT_2.yml` <br/>
...
<input type="checkbox"> Open `ASSIGNMENT_2.yml` and examine the existing structure <br/>
...
<input type="checkbox"> Fill out the `ASSIGNMENT_2.yml`, including the key components, and the steps of the action (the task you chose), under `run: |`.
...
<input type="checkbox"> Watch for the `GHA Assignment Eval` check to start running
<input type="checkbox"> This evaluator will test whether your GitHub Action runs successfully. The evaluator checks for common issues and provides guidance.
<input type="checkbox"> Wait for it to complete and comment on your PR
...
<input type="checkbox"> Permission errors: May need to add GitHub secrets or tokens (see Assignment 3 for more information if necessary)<br/>
...
<input type="checkbox"> Make changes to your `ASSIGNMENT_2.yml` file on your branch<br/>
...
Note that if you are not taking this course through Coursera or Leanpub for certification then you can ignore this step
